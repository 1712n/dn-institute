...
type Env = {
  ...
  WEBHOOK_SECRET: string; // GitHub webhook secret for HMAC verification
};
...
app.post("/webhook", async (c) => {
  ...
  try {
    payload = JSON.parse(rawBody) as GitHubCommentEvent;
  } catch {
  ...
  c.executionCtx.waitUntil(
    (async () => {
      try {
        ...
        // Run article check on all .md files under incidents/
        const mdFiles = parsedDiff.filter((f) =>
          f.header.includes("content/research/cyberattacks/incidents/") &&
          f.header.includes(".md")
        );

        const filesToCheck = mdFiles.length > 0 ? mdFiles : [parsedDiff[0]];
        const reviews: string[] = [];

        for (const file of filesToCheck) {
          const diffContent =
            file.header +
            file.body.map((s) => s.header + s.body).join("");

          // Limit diff size to avoid runaway Claude bills
          if (diffContent.length > 50000) {
            reviews.push(`⚠️ Diff too large (${diffContent.length} chars), skipping.`);
            continue;
          }
